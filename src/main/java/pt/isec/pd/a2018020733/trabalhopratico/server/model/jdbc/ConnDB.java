...
public class ConnDB {
    ...
    public List<UserInformation> getAllUsers() throws SQLException, JSONException {
        ...
        while (resultSet.next()) {
            ...
            UserInformation user = new UserInformation(
                    resultSet.getString(TableUtilizador.USERNAME.label),
                    resultSet.getString(TableUtilizador.NOME.label),
                    resultSet.getInt(TableUtilizador.ADMINISTRADOR.label),
                    resultSet.getInt(TableUtilizador.AUTENTICADO.label)
            );
    ...
    public LoginStatus getUserType(String username) throws SQLException {
        ...
        while (resultSet.next())
            if (resultSet.getInt(TableUtilizador.ADMINISTRADOR.label) == 1)
                return LoginStatus.SUCCESSFUL_ADMIN_USER;
            else
                return LoginStatus.SUCCESSFUL_NORMAL_USER;
...
//TODO: removido atualização flag autenticado na base de dados
/*
 ...
 */
    ...
    public List<Espetaculo> pesquisarEspetaculo(String dataInicio, String dataFim, boolean admin) throws SQLException, ParseException {
        ...
        if (!admin)
            sqlQuery += " WHERE " + TableEspetaculo.VISIVEL.label + "='1'";
        ...
        while (resultSet.next()) {
            ...
            if (dataInicio != null && dataFim != null) {
                if (espetaculo.getData().compareTo(Constants.formatterDate.parse(dataInicio)) > 0
                        && espetaculo.getData().compareTo(Constants.formatterDate.parse(dataFim)) < 0)
                    espetaculos.add(espetaculo);
            } else
                espetaculos.add(espetaculo);
        }
